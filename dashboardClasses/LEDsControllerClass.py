...
class LEDsController:
    def buildFrame(self, frame, MQTTClient):
        ...
        self.ledsControlFrame = tk.LabelFrame(
            frame, text="LEDs control", padx=5, pady=5
        )

        self.v = tk.StringVar()
        s1r7 = tk.Radiobutton(
            self.ledsControlFrame,
            text="LED sequence START/STOP",
            variable=self.v,
            value=1,
        ).grid(column=2, row=2, columnspan=3)
        s1r8 = tk.Radiobutton(
            self.ledsControlFrame,
            text="LED sequence for N seconds",
            variable=self.v,
            value=2,
        ...
        self.ledControlButton = tk.Button(
            self.ledsControlFrame,
            text="Start",
            bg="red",
            fg="white",
    ...
    def LEDControlButtonClicked(self):
        ...
        if self.v.get() == "2":
            self.client.publish(
                "dashBoard/LEDsService/LEDsSequenceForNSeconds", self.seconds.get()
            )
